...
insert into question_versions (question_id, question_text, tutor_id, version_number) values
(3, 'как работает алгоритм быстрой сортировки и какая у него сложность?', 3, 1),
(3, 'как работает алгоритм быстрой сортировки? объясните принцип работы и примеры', 4, 2),
(6, 'какие методы защиты от sql-инъекций существуют?', 6, 1),
(6, 'какие методы защиты от sql-инъекций существуют в веб-приложениях?', 7, 2),
(9, 'как спроектировать rest api для системы вопросов и ответов?', 9, 1),
(9, 'как спроектировать rest api для системы q&a с аутентификацией?', 10, 2),
(12, 'что такое индексы в базах данных и зачем они нужны?', 12, 1),
(12, 'что такое индексы в бд и как они улучшают производительность запросов?', 13, 2),
(15, 'как настроить ci/cd пайплайн?', 15, 1),
(15, 'как настроить ci/cd пайплайн для golang приложения?', 16, 2),
(18, 'какие паттерны проектирования использовать в микросервисах?', 18, 1),
(18, 'какие паттерны проектирования использовать в микросервисной архитектуре?', 19, 2);

insert into answer_versions (answer_id, answer_text, tutor_id, version_number) values
(3, 'алгоритм быстрой сортировки работает по принципу "разделяй и властвуй": выбирается опорный элемент, массив разбивается на элементы меньше и больше опорного, процесс рекурсивно повторяется для подмассивов', 4, 1),
(3, 'быстрая сортировка (quicksort) - это эффективный алгоритм сортировки со средней сложностью o(n log n). принцип: выбор опорного элемента, разделение массива и рекурсивная сортировка частей', 5, 2),
(6, 'методы защиты от sql-инъекций: использование подготовленных запросов (prepared statements), валидация входных данных, ограничение прав доступа бд, экранирование специальных символов', 7, 1),
(6, 'защита от sql-инъекций: параметризованные запросы, stored procedures, whitelist валидация, минимальные привилегии бд, web application firewall', 8, 2),
(9, 'rest api для системы q&a: get /questions, post /questions, get /questions/{id}, put /questions/{id}, get /questions/{id}/answers, post /answers', 10, 1),
(9, 'api дизайн для q&a системы: get /questions (список), post /questions (создать), get /questions/{id} (детали), patch /questions/{id} (обновить), get /questions/{id}/answers (ответы), post /answers (добавить ответ)', 11, 2),
(12, 'индексы в бд - это структуры данных, ускоряющие поиск. они работают как оглавление в книге, но замедляют операции вставки и обновления из-за поддержки индекса', 13, 1),
(12, 'индексы в базах данных создают дополнительные структуры (b-деревья, хэш-индексы) для ускорения поиска. trade-off: ускорение read операций за счет замедления write операций', 14, 2),
(15, 'ci/cd пайплайн настраивается в gitlab ci, github actions, jenkins. основные этапы: сборка, тестирование, анализ кода, деплой в среду', 16, 1),
(15, 'ci/cd процесс: build → test → security scan → deploy to staging → integration tests → deploy to production. инструменты: github actions, gitlab ci, circleci', 17, 2),
(18, 'паттерны для микросервисов: api gateway, circuit breaker, service discovery, event sourcing, cqrs, saga для распределенных транзакций', 19, 1),
(18, 'микросервисные паттерны: api gateway (единая точка входа), service mesh (traffic management), event-driven architecture, database per service, health checks', 20, 2);